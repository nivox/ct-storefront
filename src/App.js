...
function App() {
  ...
  async function triggerSearchPagination(newPage) {
    try {
  ...
  async function init() {
    try {
      ...
      if (cookies.config && cookies.config.ignoredAttributes) {
        (cookies.config.ignoredAttributes[projectKey] || []).forEach(a => productTypeAttributes.setIgnoreAttribute(a, true));
      }
  ...
  function handleIgnoreAttributes(e) {
                          ...
                          <Form.Select multiple onChange={handleIgnoreAttributes}>
                            {
                              ...
                              .map(a => <option key={a.name} value={a.name} selected={a.ignored}>{a.label[selectedLanguage]} ({a.name})</option>)) || []
                            }
                          </Form.Select>
                        </Modal.Body>
                      </Modal>
